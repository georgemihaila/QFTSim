...
export const useElectricFieldRepulsion = (particles: Particle[]) => {
    for (let i = 0; i < particles.length; i++) {
        ...
        if (!particle_i.enabled || !particle_i.properties.position || !particle_i.properties.mass || particle_i.properties?.mass === 0 || !particle_i.properties.chargeEV || particle_i.properties?.chargeEV === 0) {
            continue
        }
        ...
        for (let j = i + 1; j < particles.length; j++) {
            ...
            if (!particle_j.enabled || !particle_j.properties.position || !particle_j.properties.chargeEV || !particle_j.properties?.mass || particle_j.properties?.chargeEV === 0) {
                continue
            }
            ...
            const r = Math.max(distance.length(), 1e-11)// distance between the two particles
